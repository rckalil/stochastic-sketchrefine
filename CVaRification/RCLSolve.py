...
class RCLSolve:
    ...
    def __add_total_investment_constraint(self):
        num_assets = self.__no_of_vars
        
        # 1. Coeficientes da soma (c_i = 1 para todos)
        coefficients = [1] * num_assets
        
        # 2. Expressão Linear (LHS): SUM(1 * x_i)
        lhs = gp.LinExpr(coefficients, self.__vars)
        
        # 3. Adiciona a Restrição Final: SUM(x_i) == d
        self.__model.addLConstr(
            lhs, 
            GRB.EQUAL, 
            num_assets, 
            name="Total_Weight_Sum_Constraint"
        )
    ...
    def __add_constraints_to_model(
        ...
        # self.__add_total_investment_constraint()
        for constraint in self.__query.get_constraints():
