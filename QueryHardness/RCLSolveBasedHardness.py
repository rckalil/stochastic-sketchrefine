...
class RCLSolveBasedHardness:
    ...
    def __init__(
        ...
        self.__init_model_probability()


    def __init_model_probability(self):
        self.__max_tuples_in_package = \
            Hyperparameters.MAX_TUPLES_IN_PACKAGE
        self.__min_tuples_in_package = 1
        ...
        for constraint in self.__query.get_constraints():
            if constraint.is_repeat_constraint():
                if constraint.get_inequality_sign() == \
                    ...
                    self.__min_tuples_in_package =\
                        constraint.get_sum_limit()

        self.__model_probability = \
            [1.0]*(self.__max_tuples_in_package + 1)
        self.__model_probability[0] = 0
        self.__cvar_averages = []
        self.__cvar_stds = []
        self.__cvar_inequalities = []
    

    def get_model_probability(self):
        exp_package_size = \
            int((self.__min_tuples_in_package +
                self.__max_tuples_in_package)/2)
        return -np.log10(self.__model_probability[
            exp_package_size])
    

    def __update_model_probability(
            self, avg, std, gurobi_inequality, value):
        for _ in range(1, self.__max_tuples_in_package+1):
            mean = avg*_
            std = std*np.sqrt(_)
            ...
            if gurobi_inequality == GRB.GREATER_EQUAL:
                prob = 1 - prob
            print('prob:', prob)
            self.__model_probability[_] *= prob
    ...
    def __add_deterministic_constraint_to_model(
        ...
        average = np.average(self.__values[attribute])
        std = np.std(self.__values[attribute])
        self.__update_model_probability(
            avg=average, std=std,
    ...
    def __add_lcvar_constraint_to_model(
        ...
        self.__cvar_averages.append(np.average(coefficients))
        self.__cvar_stds.append(np.std(coefficients))
        self.__cvar_inequalities.append(gurobi_inequality)
            ...
            for constraint in self.__query.get_constraints():
                ...
                if all_constraints_satisfied:
                    if self.__is_objective_value_enough(
                        ...
                        for _ in range(len(cvar_mid_thresholds)):
                            print('CVaR average:', self.__cvar_averages[_])
                            print('CVaR std:', self.__cvar_stds[_])
                            self.__update_model_probability(
                                avg=self.__cvar_averages[_],
                                std=self.__cvar_stds[_],
                                gurobi_inequality=\
                                    self.__cvar_inequalities[_],
                                value=cvar_mid_thresholds[_]
                            )
                        result = CVaRificationSearchResults()
                ...
                for constraint in self.__query.get_constraints():
                    if constraint.is_risk_constraint():
                        if risk_constraint_index not in trivial_constraints:
                            ...
                            self.__cvar_averages.append(
                                np.average(coefficients))
                            self.__cvar_stds.append(
                                np.std(coefficients))                    
                        risk_constraint_index += 1
            ...
            for constraint in self.__query.get_constraints():
                ...
                if all_constraints_satisfied:
                    if self.__is_objective_value_enough(
                        ...
                        for _ in range(len(cvar_thresholds)):
                            print('CVaR avg:', self.__cvar_averages[_])
                            print('CVaR std:', self.__cvar_stds[_])
                            print('CVaR thresholds:', cvar_thresholds[_])
                            self.__update_model_probability(
                                avg=self.__cvar_averages[_],
                                std=self.__cvar_stds[_],
                                gurobi_inequality=self.__cvar_inequalities[_],
                                value=cvar_thresholds[_]
                            )
                        result = CVaRificationSearchResults()
                        result.set_found_appropriate_package(True)
                        result.set_package(package)
